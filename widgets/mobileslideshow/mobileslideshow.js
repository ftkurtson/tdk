(function ()
{
	BaseKit.Widget.Mobileslideshow = null;

	BaseKit.Widget.MobileslideshowProperties = {
		albumRef: null,
		album: [],
		aspect: 75,
		scale: 'zoom',
	...
	BaseKit.Widget.MobileslideshowMethods = {
		construct: function (el, options)
		{
			this.preLoadImages(this.properties.data.album);
			this.load();				
		},
		...
		load: function ()
		{
			...
			if ((this.properties.data.album.length > 0) || this.properties.changed.album)
			{
		...
		{		
			...
			{
				if (that.hasChangedProperties())
				{	
					//replace saved values with changed values for editor view
					$.each(that.properties.changed, function(key, value)
					{
						that.properties.data[key] = value;
					});	

					//preload the new image array
					that.preLoadImages(this.properties.data.album);
				}
			}

			data = this.properties.data;

			//if this is a re-render, preload the new images
			if (!fromLoad)
			{
				that.preLoadImages(data.album);
			}
		...
		setAspectProperties: function()
		{
			var style = document.createElement("style"),
				...
				ratio = this.get('aspect'),
				backgroundStyles = [],
				imageArray = this.properties.data.album,
	...
	BaseKit.Widget.Mobileslideshow = function ()
	{
		var o = new BaseKit.WidgetCore(this, arguments, {
			properties: BaseKit.Widget.MobileslideshowProperties,
			methods: BaseKit.Widget.MobileslideshowMethods
		});
	};

	// JQuery plugin so that a widget can be attached to an element
	$.fn.basekitWidgetMobileslideshow = function (options)
	{
		this.each(function (index, el)
		{
			$(el).data('bkob', new BaseKit.Widget.Mobileslideshow(el, options));
		});
	};
}());